...
export default function SelectPieces({ all_pieces, all_selected_pieces, num_allowed_pieces, map, this_player_id, setPieces }: Props) {
    ...
    return (
        <Stack spacing={3} justifyContent={'center'} alignItems={'center'} sx={{ position: 'relative', pt: '5%', pb: '10%' }}>
            { begin_game_ready ?
            <Button variant={'contained'} onClick={() => { endTurn(sendJsonMessage) }}>
                Begin Game
            </Button> :
            <Stack spacing={2} justifyContent={'center'}>
                <Typography variant='h5'>Pieces Selected: {selectedTeam.length}/{num_allowed_pieces}</Typography>
